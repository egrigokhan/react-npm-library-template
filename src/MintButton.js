...
export default function MintButton(props_) {
	...
	const mintNFT = async (count) => {
		if(window.ethereum) {
			...
			var contract = new web3.eth.Contract(props.ABI, props.ADDRESS);

			eval(`contract.methods.${props.mintMethodName}${props.mintMethodInput}`).send({
				from: account,
  ...
  return (
    ...
    >
				...
				<div
				class={`${props.jitter ? "jitter-container" : ""} mint-button-middle-bar-container`} onClick={(e) => {
